...
import { AppDispatch } from '@kinvolk/headlamp-plugin/lib/redux/stores/store';
import { runCommand } from '@kinvolk/headlamp-plugin/lib/components/App/runCommand';
  ...
  return (
    <CommandDialog
      ...
      title={
        askClusterName
          ? acting
            ? t('Creating Minikube Cluster')
            : t('New Minikube Cluster')
          : acting
          ? t('Running "{{ command }}" on Minikube Cluster', { command })
          : t('Running "{{ command }}" on Minikube Cluster', { command })
      }
      acting={acting}
      running={running}
